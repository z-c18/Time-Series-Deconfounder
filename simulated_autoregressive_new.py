...
class AutoregressiveSimulation:
    def __init__(self, gamma, num_simulated_hidden_confounders):
        ...
        self.covariates_coefficients['treatments'] = self.generate_coefficients(
            self.maxP, matrix_shape=(self.num_covariates, self.num_treatments), treatment_coefficients=True)

        self.covariates_coefficients['covariates'] = self.generate_coefficients(
            self.maxP, matrix_shape=(self.num_covariates, self.num_covariates), variables_coefficients=True)

        self.confounders_coefficients = dict()
        self.confounders_coefficients['treatments'] = self.generate_coefficients(
            self.maxP, matrix_shape=(self.num_confounders, self.num_treatments))
        self.confounders_coefficients['confounders'] = self.generate_coefficients(
            self.maxP, matrix_shape=(self.num_confounders, self.num_confounders), variables_coefficients=True)
    ...
    def generate_coefficients(self, p, matrix_shape, variables_coefficients=False, treatment_coefficients=False):
        ...
        for i in range(p):
            if (variables_coefficients):
                diag_elements = [np.random.normal(1.0, 1.0 / p) for _ in range(matrix_shape[0])]
                # x (1.0 - (i+1) * 1.0 / p, 1.0 / p)
    ...
    def generate_treatment_assignments_single_timestep(self, p, history):
        ...
        for index in range(t):
            average_covariates = average_covariates + covariates_history[history_length - index - 1] / t * p
            avearge_confounders = avearge_confounders + confounders_history[history_length - index - 1] / t * p
    ...
    def generate_covariates_single_timestep(self, p, history):
        ...
        for index in range(t):
            treatments_sum += np.matmul(past_treatment_coefficients[index],
                                        treatments_history[history_length - index - 1]) *  p / t * (1.0 - (times[history_length] - times[history_length - index - 1]) / p) 

            covariates_sum += np.matmul(past_covariates_coefficients[index],
                                        covariates_history[history_length - index - 1]) *  p / t * (1.0 - (times[history_length] - times[history_length - index - 1]) / p) 
    ...
    def generate_confounders_single_timestep(self, p, history):
        ...
        for index in range(t):
            ...
            confounders_sum += np.matmul(past_confounders_coefficients[index],
                                         confounders_history[history_length - index - 1]) * p / t * (1.0 - (times[history_length] - times[history_length - index - 1]) / p) 
    ...
    def generate_data_single_patient(self, timesteps, times):
        ...
        history['times'] = [times[0]]

        for t in range(timesteps):
            history['times'].append(times[t + 1])
    ...
    #         times = np.zeros(shape=(timesteps + 1))
    #         times[1:timesteps] = np.random.uniform(0, timesteps, size=(timesteps - 1))
    #         times[-1] = timesteps
    #         times = np.sort(times)
    ...
    def generate_dataset(self, num_patients, max_timesteps, binary_outcome=False):
            ...
            for patient in range(num_patients):
                ...
                times = np.zeros(shape=(timesteps + 2))
                times[1:timesteps + 1] = np.random.uniform(0, timesteps + 1, size=(timesteps))
                times[-1] = timesteps + 1
                times = np.sort(times)
                ...
                times = np.vstack((np.array(times),
                np.zeros(shape=(max_timesteps-timesteps, 1))))
                ...
                dataset['times'].append(np.array(times))
